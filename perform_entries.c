...
static char* WBConfigForgeDownloadURL(const WBEntry* wbe,
    ...
    printf("\n*url_size=%"PRIuPTR"and sizeof(\"%%s\")%"PRIuPTR,url_size,sizeof("%s"));
char*url=calloc(url_size + 1, sizeof(char));
snprintf(url,url_size,DOWNLOAD_URL_MASK,wbcred->wallabag_host,wbe->id,wbcred->token);
return url;
}


static char*GetEntryFileName(const WBEntry*wbe){
...
static void  PerformEverything(const WBEntry*wbe,const WBoAuthCred*wbcred){
char*filename=GetEntryFileName(wbe);
